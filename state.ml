...
let wealthiest_player brd st = 
  let rec max_wealth brd st =
    ...
    | (p,invs)::t -> let st' = {st with curr_player = p} in 
      ...
      if curr_wealth > snd (List.hd xs) then 
        (current_player st',curr_wealth) :: []
...
let has_prop prop st = 
  (List.mem prop (List.assoc st.curr_player st.inventories))


let sell bd prop st = 
  if is_buyable bd prop && not (is_in_jail st) && has_prop prop st then
    match (earn_cash st (cost bd prop)) with 
    | Legal st' -> 
      let curr_invent = List.assoc st.curr_player st.inventories in 
      let trimmed = List.remove_assoc st.curr_player st.inventories in 
      let new_inv = (st.curr_player, List.filter 
                       (fun p -> p <> prop) curr_invent) :: trimmed in 
      Legal {
        st' with inventories = new_inv
      }
    | _ -> Illegal 
...
let pay_rent bd prop st =  
  ...
  if pay_to = 0 || pay_to = (current_player st) then Legal st else 
    let pay_amt = (rent bd prop) + ((rent bd prop) * ((houses st prop) + 
                                                      (hotels st prop))) 
    in match earn_cash st (-pay_amt) with 
    | Legal st1 -> 
      ...
      Legal {st1 with wallets = new_cash}
    | _ -> Illegal


(** [has_monopoly bd prop st] return true if the current player has a monopoly
    containing [prop] *)
let has_monopoly bd prop st= 
  let monopoly_group = monopoly_group_named bd prop in
  List.for_all (fun s -> List.mem s (curr_player_inventory st)) monopoly_group


let build_houses bd st prop n  = 
  if is_buildable bd prop && not (is_in_jail st) && has_monopoly bd prop st then 
    let house_cost = match house_cost bd prop with 
      | Some v -> v * n
      | None -> raise (Unbuildable prop) in
    if not (List.assoc st.curr_player st.wallets >= n * house_cost) then Illegal 
    else let curr_houses = houses st prop in 
      if (curr_houses + n) <= 3 then 
        ...
        let st1 = {
          st with buildings = new_buildings
        } in 
...
let build_hotels bd st prop n  = 
  if is_buildable bd prop && not (is_in_jail st) && houses st prop = 3 then 
    let hotel_cost = match (hotel_cost bd prop) with 
      | Some v -> n * v 
      | None -> raise (Unbuildable prop) in  
    if not (List.assoc st.curr_player st.wallets >= n * hotel_cost) then Illegal 
    else let curr_hotels = hotels st prop in
      if (curr_hotels + n) <= 3 then 
        let curr_houses = houses st prop in 
        let trimmed = List.remove_assoc prop st.buildings in 
        let new_buildings = (prop, (curr_houses-3, curr_hotels+n)) :: trimmed in 
        let st1 = { st with buildings = new_buildings } in 
...
let card_action bd cd st = 
  ...
  | Location -> 
    ...
    let st0 = {st with locations = new_locations} in 
    let st1 = {st0 with in_jail = jail} in 
  ...
  | LeaveJail -> 
    ...
    let st1 = {st with items = new_items} in 
    Legal {st1 with in_jail = new_jail}
...
let move_cards brd crd st = 
  ...
  | LeaveJail -> 
    ...
    let st0 =  { st with items = new_items} in 
    Legal {st0 with cards = new_cards}
